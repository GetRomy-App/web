...
<svelte:head>
	...
	<meta
		name="keywords"
		content="{title} Rōmy, GetRomy LLC legal, nonprofit software policy, donor intelligence terms"
	/>
	...
	{@html `<script type="application/ld+json">${JSON.stringify({
		...
		itemListElement: [
			...
			{ '@type': 'ListItem', position: 3, name: title, item: `https://getromy.app${canonicalPath}` }
		]
	})}</script>`}
</svelte:head>
