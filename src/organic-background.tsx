...
export function OrganicBackground({
  ...
  // Smoothly transition colors when isDark changes
  useEffect(() => {
    const material = materialRef.current;
    if (!material) return;

    const target = isDark ? DARK_COLORS : LIGHT_COLORS;
    const from = {
      c1: material.uniforms.uColor1.value.clone(),
      c2: material.uniforms.uColor2.value.clone(),
      c3: material.uniforms.uColor3.value.clone(),
      c4: material.uniforms.uColor4.value.clone(),
    };
    const to = {
      c1: new THREE.Color(target.color1),
      c2: new THREE.Color(target.color2),
      c3: new THREE.Color(target.color3),
      c4: new THREE.Color(target.color4),
    };
    ...
    function lerp(elapsed: number) {
      const t = Math.min(elapsed / duration, 1);
      const ease = t * t * (3 - 2 * t); // smoothstep
      material!.uniforms.uColor1.value.copy(from.c1).lerp(to.c1, ease);
      material!.uniforms.uColor2.value.copy(from.c2).lerp(to.c2, ease);
      material!.uniforms.uColor3.value.copy(from.c3).lerp(to.c3, ease);
      material!.uniforms.uColor4.value.copy(from.c4).lerp(to.c4, ease);
      if (t < 1) {
        raf = requestAnimationFrame(() => lerp(performance.now() - start));
      }
    }
    lerp(0);
