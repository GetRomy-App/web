...
function DialogContent({ className, ...props }: DialogContentProps) {
  return (
    <DialogPrimitive.Portal>
      <DialogPrimitive.Backdrop className="fixed inset-0 bg-neutral-900/30 dark:bg-black/60 backdrop-blur-sm" />
      <DialogPrimitive.Popup
        className={
          ...
          "dark:border-neutral-800 dark:bg-neutral-950 dark:shadow-2xl dark:shadow-purple-500/5 " +
          (className ?? "")
        }
        {...props}
      />
...
function DialogTitle({ className, ...props }: DialogTitleProps) {
  return (
    <DialogPrimitive.Title
      className={"text-lg font-[450] text-neutral-900 dark:text-neutral-100 " + (className ?? "")}
      {...props}
    />
  );
}
