...
<svelte:head>
	<!-- Title, description, keywords, canonical, OG & Twitter tags for this route are rendered
	     once from the root layout — see src/routes/+layout.svelte. -->
	...
	{@html `<script type="application/ld+json">${JSON.stringify({
		...
		description:
			'Composite accuracy benchmark comparing Rōmy against ChatGPT, Claude, and Gemini on AI-powered donor prospect research across factual precision, discovery recall, hallucination rate, capacity estimation, source attribution, structural completeness, and actionability.',
		url: 'https://getromy.app/labs',
		creator: { '@type': 'Organization', name: 'Rōmy', url: 'https://getromy.app' },
		variableMeasured: PIF_OVERALL.bars.map((b) => `${b.name}: ${b.score}`)
	})}</script>`}
	...
	{@html `<script type="application/ld+json">${JSON.stringify({
		...
		url: 'https://getromy.app/labs',
		description:
			'Technical deep-dives, research findings, and perspectives on nonprofit fundraising, benchmarked against enterprise wealth screening platforms.',
		publisher: { '@type': 'Organization', name: 'Rōmy', url: 'https://getromy.app' },
	...
	>
		...
		<section class="px-4 md:px-8 py-8 relative flex w-full flex-col">
			...
			<div class="flex flex-col items-start justify-center">
				...
				>
					Try Rōmy on your own prospect list. See how purpose-built donor intelligence compares to
