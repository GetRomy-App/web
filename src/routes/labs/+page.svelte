<script lang="ts">
	...
	import { postListJsonLd, breadcrumbJsonLd } from '$lib/seo';

	let { data } = $props();

	const jsonLdBlog = postListJsonLd(
		'Rōmy Labs',
		'PIF-Bench results and technical deep-dives comparing purpose-built AI donor intelligence to enterprise tools and manual research.',
		'/labs',
		data.posts,
		'/labs/blog'
	);
...
<svelte:head>
	...
	<meta
		name="keywords"
		content="donor research benchmark, AI prospect research comparison, nonprofit fundraising AI, wealth screening accuracy, PIF-Bench, donor intelligence cost comparison, ChatGPT vs Rōmy, prospect research tool, AI donor research, major donor prospecting"
	/>
	...
	{@html `<script type="application/ld+json">${JSON.stringify(jsonLdBlog)}</script>`}
