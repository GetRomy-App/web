<script lang="ts">
	...
	/** Built from the live PIF_OVERALL data so the meta description can never drift from the chart on the page. */
	const scoreByName = Object.fromEntries(PIF_OVERALL.bars.map((b) => [b.name, b.score]));
	const benchSummary = `Rōmy scored ${scoreByName['Romy']} on PIF-Bench vs. ChatGPT (${scoreByName['ChatGPT']}), Claude (${scoreByName['Claude']}), and Gemini (${scoreByName['Gemini']}).`;
	const benchSummaryShort = `PIF-Bench results: Rōmy ${scoreByName['Romy']}, Claude ${scoreByName['Claude']}, ChatGPT ${scoreByName['ChatGPT']}, Gemini ${scoreByName['Gemini']}. Open benchmarks for AI-powered prospect research.`;
...
<svelte:head>
	...
	<meta
		name="description"
		content="{benchSummary} See how purpose-built AI donor intelligence compares on accuracy, cost, and speed."
	/>
	<meta
		name="keywords"
		content="donor research benchmark, AI prospect research comparison, nonprofit fundraising AI, wealth screening accuracy, PIF-Bench, donor intelligence cost comparison, ChatGPT vs Rōmy, AI prospect research tool, nonprofit AI benchmark"
	/>
	<meta property="og:title" content="Rōmy Labs — AI Donor Research Benchmarks" />
	<meta property="og:description" content={benchSummaryShort} />
	...
	<meta name="twitter:description" content={benchSummaryShort} />
