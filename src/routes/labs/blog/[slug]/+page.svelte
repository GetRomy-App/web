<script lang="ts">
	...
	const labsPostingSchema = $derived(JSON.stringify({
		...
		"inLanguage": "en-US",
		"about": [
			{"@type": "Thing", "name": "AI Donor Research Benchmark"},
	...
	const labsBreadcrumbSchema = $derived(JSON.stringify({
		"@context": "https://schema.org",
		"@type": "BreadcrumbList",
		"itemListElement": [
...
<svelte:head>
	...
	<meta name="keywords" content="AI donor research benchmark, nonprofit prospect research, wealth screening accuracy, PIF-Bench, donor intelligence comparison, AI fundraising tools, nonprofit AI benchmark, prospect research accuracy, {data.post.tag.toLowerCase()}" />
	...
	{@html `<script type="application/ld+json">${labsPostingSchema}</script>`}
	{@html `<script type="application/ld+json">${labsBreadcrumbSchema}</script>`}
</svelte:head>
