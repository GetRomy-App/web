<script lang="ts">
	...
	import { deriveKeywords, blogPostingJsonLd, breadcrumbJsonLd } from '$lib/seo';

	let { data } = $props();

	const keywords = deriveKeywords(data.post.title, data.post.tag, data.post.excerpt);
	const postUrl = `https://getromy.app/labs/blog/${data.post.slug}`;
	const jsonLdArticle = blogPostingJsonLd(data.post, `/labs/blog/${data.post.slug}`);
	const jsonLdBreadcrumb = breadcrumbJsonLd([
		{ name: 'Home', path: '/' },
		{ name: 'Labs', path: '/labs' },
		{ name: data.post.title, path: `/labs/blog/${data.post.slug}` }
	]);
