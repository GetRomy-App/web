<script lang="ts">
	...
	const tagKeywords: Record<string, string[]> = {
		'Field Notes': ['major donor cultivation', 'donor relationships', 'nonprofit development', 'fundraising stories', 'nonprofit major gifts', 'donor stewardship', 'fundraising best practices', 'development director'],
		'Industry': ['nonprofit industry trends', 'fundraising industry', 'philanthropy', 'nonprofit sector', 'donor research trends', 'major gifts strategy', 'nonprofit leadership'],
		'Engineering': ['AI engineering', 'donor intelligence technology', 'machine learning fundraising', 'nonprofit tech', 'prospect research AI'],
		'Research': ['PIF-Bench', 'AI benchmark', 'donor research accuracy', 'wealth screening comparison', 'ChatGPT vs Rōmy', 'AI prospect research study', 'nonprofit AI benchmark'],
		'Data Science': ['donor data science', 'wealth indicators', 'giving capacity scoring', 'philanthropic data', 'nonprofit analytics', 'predictive giving']
	};

	function getPostKeywords(tag: string): string {
		const base = ['nonprofit fundraising', 'donor intelligence', 'Rōmy', 'prospect research', 'major donor', 'AI donor research', 'wealth screening'];
		const specific = tagKeywords[tag] ?? [];
	...
	onMount(() => {
		...
		const init = async () => {
			...
			// Animate benchmark bar charts if present
			if (chartsSection) {
...
<svelte:head>
	...
	<meta name="keywords" content={getPostKeywords(data.post.tag)} />
