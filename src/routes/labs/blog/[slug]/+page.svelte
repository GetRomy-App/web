<script lang="ts">
	...
	import { SITE, OG_IMAGE, postKeywords, blogPostingJsonLd, breadcrumbJsonLd } from '$lib/seo';

	let { data } = $props();

	const postUrl = `${SITE}/labs/blog/${data.post.slug}`;
	const keywords = postKeywords(data.post.title, data.post.tag, data.post.excerpt);
	const jsonLdPost = blogPostingJsonLd({
...
<svelte:head>
	...
	{@html `<script type="application/ld+json">${JSON.stringify(jsonLdPost)}</script>`}
