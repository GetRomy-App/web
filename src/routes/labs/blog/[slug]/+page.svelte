...
<svelte:head>
	...
	<meta name="keywords" content="{data.post.tag}, AI donor research benchmark, prospect research comparison, donor intelligence accuracy, wealth screening AI, nonprofit fundraising AI" />
	...
	{@html `<script type="application/ld+json">${JSON.stringify({
		...
		itemListElement: [
			...
			{ '@type': 'ListItem', position: 2, name: data.post.title, item: `https://getromy.app/labs/blog/${data.post.slug}` }
		]
	})}</script>`}
</svelte:head>
