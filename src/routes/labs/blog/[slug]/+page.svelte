<script lang="ts">
	...
	const articleSchema = $derived(JSON.stringify({
		...
		publisher: {
			...
			logo: { '@type': 'ImageObject', url: 'https://getromy.app/favicon-96x96.png', width: 96, height: 96 }
		},
		image: { '@type': 'ImageObject', url: 'https://getromy.app/og-image.jpg', width: 1200, height: 630 },
		mainEntityOfPage: { '@type': 'WebPage', '@id': `https://getromy.app/labs/blog/${data.post.slug}` },
		articleSection: data.post.tag,
		keywords: `AI donor research benchmark, prospect research fidelity, PIF-Bench, ${data.post.tag.toLowerCase()}`,
		isPartOf: { '@type': 'Blog', name: 'Rōmy Labs', url: 'https://getromy.app/labs' }
	}));
...
<svelte:head>
	...
	<meta name="keywords" content="AI donor research benchmark, prospect research accuracy, nonprofit AI tools, donor intelligence comparison, PIF-Bench, wealth screening benchmark, AI fundraising comparison, {data.post.tag.toLowerCase()}, ChatGPT prospect research, donor research fidelity" />
	...
	<meta property="article:tag" content="donor intelligence benchmark" />
