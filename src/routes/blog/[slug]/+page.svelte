<script lang="ts">
	...
	const tagKeywords: Record<string, string> = {
		'Engineering': 'fundraising engineering, AI engineering nonprofit, donor intelligence engineering, nonprofit software development, machine learning fundraising, prospect research technology',
		'Research': 'fundraising research, nonprofit research findings, donor research methodology, prospect research analysis, AI research nonprofits, philanthropic data research',
		'Data Science': 'nonprofit data science, donor scoring models, predictive fundraising analytics, wealth modeling, giving capacity prediction, donor analytics platform',
		'Industry': 'nonprofit industry insights, fundraising industry trends, major gifts strategy, donor cultivation, capital campaign planning, nonprofit sector analysis',
		'Field Notes': 'nonprofit field notes, major gift fundraising stories, donor relationship building, development officer insights, frontline fundraising, donor stewardship'
	};
	const baseKeywords = 'donor intelligence, nonprofit fundraising, prospect research, AI donor research, wealth screening, small nonprofits, Rōmy, major donors';
	const postKeywords = `${data.post.title}, ${baseKeywords}, ${tagKeywords[data.post.tag] ?? data.post.tag.toLowerCase()}`;

	const postUrl = `https://getromy.app/blog/${data.post.slug}`;
