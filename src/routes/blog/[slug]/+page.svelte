<script lang="ts">
	...
	const articleSchema = $derived(JSON.stringify({
		...
		mainEntityOfPage: { '@type': 'WebPage', '@id': `https://getromy.app/blog/${data.post.slug}` },
		articleSection: data.post.tag,
		keywords: `donor intelligence, nonprofit fundraising, prospect research, ${data.post.tag.toLowerCase()}`,
...
<svelte:head>
	...
	<meta name="keywords" content="donor intelligence, nonprofit fundraising, prospect research, AI donor research, wealth screening, major donor identification, {data.post.tag.toLowerCase()}, nonprofit development, philanthropic research, giving history" />
