<script lang="ts">
	...
	import { blogPostingSchema, breadcrumbSchema, buildPostKeywords, jsonLd } from '$lib/seo';

	let { data } = $props();

	const postUrl = `https://getromy.app/blog/${data.post.slug}`;
	const postKeywords = buildPostKeywords(data.post);
	const articleSchema = blogPostingSchema(data.post, postUrl);
	const breadcrumbs = breadcrumbSchema([
		{ name: 'Home', url: 'https://getromy.app/' },
		{ name: 'Blog', url: 'https://getromy.app/blog' },
