...
<svelte:head>
	...
	<meta name="keywords" content="{data.post.tag}, donor intelligence, nonprofit fundraising, prospect research, AI donor research, wealth screening, major donor prospecting, small nonprofit fundraising" />
	...
	{@html `<script type="application/ld+json">${JSON.stringify({
		...
		'@type': 'BreadcrumbList',
		itemListElement: [
			{ '@type': 'ListItem', position: 1, name: 'Blog', item: 'https://getromy.app/blog' },
			{ '@type': 'ListItem', position: 2, name: data.post.title, item: `https://getromy.app/blog/${data.post.slug}` }
		]
	})}</script>`}
</svelte:head>
