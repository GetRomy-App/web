<script lang="ts">
	...
	import { SITE, DEFAULT_OG_IMAGE, breadcrumbList } from '$lib/seo';
	...
	const blogItemList = {
		...
		'@type': 'ItemList',
		itemListElement: data.posts.map((post: { title: string; slug: string }, i: number) => ({
			'@type': 'ListItem',
			position: i + 1,
			name: post.title,
...
<svelte:head>
	...
	<meta
		name="keywords"
		content="nonprofit fundraising blog, donor intelligence, prospect research, AI for nonprofits, fundraising insights, major gift fundraising, wealth screening, donor retention, planned giving"
	/>
	...
	{@html `<script type="application/ld+json">${JSON.stringify(blogItemList)}</script>`}
</svelte:head>
