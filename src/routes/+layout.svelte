...
<!--
	This layout intentionally does NOT set <title>, meta description/keywords, canonical,
	or og:url/title/description/twitter:url/title/description — every route sets its own
	via its own <svelte:head>. Svelte's <svelte:head> merge order is unreliable when the
	same tag is declared in both a layout and a page, so page-identity tags must be owned
	by exactly one place (the page) to avoid every route silently inheriting the homepage's
...
<svelte:head>
	...
	<!-- Open Graph / Twitter defaults — pages may add their own og:title etc.,
	     but the share image and card type are the same across the whole site. -->
	...
	<!-- Structured Data (JSON-LD) — sitewide publisher identity. Page-specific schema
	     (SoftwareApplication, BlogPosting, BreadcrumbList, etc.) lives on each route. -->
