...
<!--
	Only truly site-wide tags live here — anything that should differ per route (title,
	description, keywords, robots, canonical, og:/twitter: tags) belongs in that route's own
	<svelte:head>. A <title>/<meta> declared here would compete with every page's own tags in
	the rendered <head>, and pages would silently lose that fight — see the homepage's
	<svelte:head> for the pattern every other route already follows.
-->
<svelte:head>
	<meta charset="utf-8" />
	...
	<!-- Structured Data (JSON-LD) — organization-level facts, valid on every page -->
