<script lang="ts">
	...
	// NOTE: title, description, canonical, Open Graph/Twitter, and page-identity JSON-LD
	// (SoftwareApplication) live on each individual +page.svelte, not here. Svelte's SSR
	// head rendering keeps only one <title>/duplicate-<meta> per document, and this layout
	// wraps every route — defining page-identity tags here previously caused every single
	// page on the site (blog posts, labs, legal, etc.) to render the homepage's title and
	// description instead of its own. Only truly global, route-independent tags belong here.
	const description =
...
<svelte:head>
	...
	<!-- Structured Data (JSON-LD) — global, org-level entities only. Page-identity schema
	     (SoftwareApplication, BlogPosting, etc.) lives on each individual +page.svelte. -->
