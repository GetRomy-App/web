<script lang="ts">
	...
	// Only truly site-wide tags live here. Page-specific title/description/canonical/OG
	// tags are set by each route — Svelte's SSR head merging doesn't reliably let a page
	// override a tag the root layout also sets (duplicate <title>/<link rel=canonical>
	// confuse crawlers into treating every route as a homepage duplicate), so this layout
	// must not declare any tag that a page needs to own.
</script>
...
<svelte:head>
	...
	<!-- Open Graph / Twitter defaults (site-wide; pages set their own title/description/url/type) -->
