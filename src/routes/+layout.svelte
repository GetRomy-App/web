...
<svelte:head>
	...
	<!-- Open Graph / Twitter defaults — each route sets its own title, description,
	     canonical, and og:/twitter: tags in its own <svelte:head>. Only truly
	     site-wide, non-conflicting tags belong here: putting page-specific tags in
	     the root layout previously caused every route to inherit the homepage's
	     <title>, description, canonical, and og:url (nested <svelte:head> content
	     is appended, not replaced), so every blog post shared the same search
	     snippet and social preview as the homepage. -->
	...
	<!-- Structured Data (JSON-LD) — Organization entity, present site-wide so every
	     page (including blog posts crawled independently) resolves to the same
	     brand entity for knowledge-panel / GEO citation purposes. The
	     SoftwareApplication schema lives on the homepage, where it's topical. -->
