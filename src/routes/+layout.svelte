...
<svelte:head>
	...
	<!--
		Only truly page-agnostic tags live here. title / description / keywords / og:title /
		og:description / og:url / og:type / twitter:title / twitter:description / twitter:url /
		canonical are set per-page (each route has its own <svelte:head>) — Svelte concatenates
		layout + page <svelte:head> content rather than letting the page override the layout, so
		anything set here as a fixed value would leak onto every route (every blog post, /labs,
		/legal, /status, etc. would otherwise all render this layout's homepage title).
	-->
	...
	<!-- Open Graph / Twitter defaults that hold across every route -->
