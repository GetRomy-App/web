<script lang="ts">
	...
	// NOTE: title/description/canonical/OG/Twitter/robots and any structured data that
	// varies per page live in each route's own <svelte:head> (see src/routes/+page.svelte
	// for the homepage, src/lib/components/legal/LegalPage.svelte for legal docs, etc).
	// Svelte's SSR head renderer resolves <title> as a single sitewide slot, and the
	// outermost <svelte:head> to declare one wins — so putting a page's title here would
	// silently clobber every route's own title. Only truly page-independent tags belong
	// in this layout.
</script>
...
<svelte:head>
	...
	<!-- Structured Data (JSON-LD) — sitewide Organization entity, safe to repeat on every page -->
