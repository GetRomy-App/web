...
<svelte:head>
	...
	<!--
		Title, description, OG/Twitter tags, and canonical URL are intentionally NOT set here.
		Svelte's SSR head renderer keeps only one <title> (and effectively one primary
		description) across nested +layout/+page <svelte:head> blocks, resolved by a
		structural path comparison — not "child overrides parent". Declaring them in this
		shared layout silently clobbered every page's own title/description in the
		prerendered output. Each route sets its own via its +page.svelte instead; the
		homepage's defaults live in src/routes/+page.svelte.
	-->
