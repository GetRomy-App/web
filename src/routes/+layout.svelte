<script lang="ts">
	...
	// Sitewide default — the fallback social-share description, used only in the
	// WebSite structured data below. Page-level <svelte:head> blocks own their own
	// title/description/canonical; duplicating those here would race a page's own
	// tags for precedence, so this layout only ever emits tags that are identical
	// on every route.
	const description =
...
<svelte:head>
	...
	<!-- Open Graph / Twitter defaults that hold for every route -->
