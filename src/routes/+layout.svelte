<script lang="ts">
	...
	// Per-page SEO (title, description, keywords, OG/Twitter, canonical) lives in
	// every route via the <Seo> component — every routed page renders one. Do NOT
	// set a <title> or other per-page meta tag here: Svelte resolves competing
	// <svelte:head> titles by tree position, not document order, so a title set in
	// this root layout can silently beat (or lose to) a page's own <Seo> title
	// depending on nesting depth — it did, and every page shipped the homepage's
	// title until this was removed. Keep this layout to tags that are identical
