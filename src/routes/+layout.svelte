<script lang="ts">
	...
	// Every route sets its own title/description/canonical/OG in its own <svelte:head>.
	// SvelteKit concatenates layout + page head content rather than deduping by tag name,
	// so these site-wide defaults only render on "/" — otherwise they'd double up behind
	// (and potentially outrank, in some crawlers/scrapers) each page's own tags.
