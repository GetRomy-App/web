...
<svelte:head>
	...
	<!--
		Page title, description, keywords, robots, canonical, og:type/url/title/description,
		and twitter:url/title/description are intentionally NOT set here — every route sets
		its own. A shared default here would sit earlier in document order than each page's
		own tag, and per the HTML/OpenGraph spec the first occurrence of a duplicated tag wins,
		so a site-wide default would silently clobber every page's own metadata with the
		homepage's (this is what was happening before: every blog post, labs post, and legal
		page rendered with the homepage's <title> and canonical URL).
	-->
	<meta name="author" content="GetRomy LLC" />
	<meta name="theme-color" content="#0d0d0e" media="(prefers-color-scheme: dark)" />
	<meta name="theme-color" content="#fcfcfc" media="(prefers-color-scheme: light)" />

	<!-- Open Graph / Twitter defaults that don't vary per page -->
