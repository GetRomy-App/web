<script lang="ts">
	...
	// Every other route defines its own complete title/description/robots/canonical/OG/Twitter
	// tags in its own <svelte:head>. Svelte's SSR only keeps a single <title> (the "deepest"
	// one wins) but plain <meta>/<link> tags are NOT deduplicated — every layout tag below would
	// render a second time alongside each page's own tags, and since it comes first in document
	// order, link-preview crawlers (Facebook, Twitter/X, LinkedIn) that take the *first* matching
	// tag would pick up this generic homepage copy instead of the page-specific one. So this
	// block only renders on '/', where there is no page-level override to collide with.
