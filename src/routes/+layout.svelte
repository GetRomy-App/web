...
<svelte:head>
	...
	<!--
		Title, description, keywords, robots, canonical, and Open Graph/Twitter tags are
		intentionally NOT set here. Svelte's SSR only keeps one <title> per render, and it
		favors whichever <svelte:head> registers first — since this layout wraps every page,
		a title set here silently wins over every page's own title, description, and
		canonical link. Each route sets its own instead.
	-->
	...
	<!-- Structured Data (JSON-LD): site-wide entity data, additive to each page's own schema -->
