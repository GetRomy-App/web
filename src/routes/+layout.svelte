...
<svelte:head>
	...
	<!--
		Sitewide defaults only — title, description, canonical, og:*, and twitter:* live on each
		+page.svelte instead. Svelte merges layout + page <svelte:head> content, so anything
		page-specific declared here would render twice (once from the layout, once from the page)
		and give crawlers two conflicting <title>/canonical/og:url tags per page.
	-->
	...
	<!-- Open Graph / Twitter defaults (no page overrides these) -->
	...
	<!-- Structured Data (JSON-LD) — Organization is sitewide; page-specific schema (product,
	     article, breadcrumbs) lives on the relevant +page.svelte -->
