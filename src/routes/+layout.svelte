<script lang="ts">
	...
	// Sitewide title/description/OG/Twitter/canonical are no longer set here — every
	// route now owns its own via <Seo>, which avoids the duplicate <meta> tags that
	// used to stack on top of these defaults on every non-home page.
</script>
...
<svelte:head>
	...
	<!-- Structured Data (JSON-LD) — sitewide brand identity. Page-specific schema
	     (SoftwareApplication, Article, BreadcrumbList) lives on each route via <Seo>. -->
