...
<svelte:head>
	<!--
		Only truly sitewide, non-conflicting tags belong here. Title, description,
		keywords, og:type/url/title/description, twitter:url/title/description, and
		rel=canonical are page-specific and must be set by each route's own
		svelte:head — SvelteKit concatenates nested svelte:head blocks rather than
		replacing them, so a second definition here would render a duplicate,
		competing tag on every page instead of overriding it.
	-->
	<meta name="author" content="GetRomy LLC" />
	<meta name="theme-color" content="#0d0d0e" media="(prefers-color-scheme: dark)" />
	<meta name="theme-color" content="#fcfcfc" media="(prefers-color-scheme: light)" />

	<!-- Open Graph / Twitter defaults (identical on every page, so safe to set once) -->
