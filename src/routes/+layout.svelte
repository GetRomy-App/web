<script lang="ts">
	...
	// Site-wide description, reused by the JSON-LD below. Page-specific <title>/description/
	// canonical/OG/Twitter tags live on each route's own +page.svelte (see src/routes/+page.svelte
	// for the homepage) — a single <svelte:head><title> per rendered tree is the only way to
	// guarantee the browser and search crawlers pick up the *right* title for that route.
	const description =
...
<svelte:head>
	...
	<!-- Open Graph / Twitter identity that doesn't vary by page -->
	...
	<link
		...
		title="Rōmy Blog"
		href="https://getromy.app/rss.xml"
	/>
