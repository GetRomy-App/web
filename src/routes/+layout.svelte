...
<!--
	Only truly site-wide tags live here. Page-specific tags (title, description,
	keywords, og:url/title/description, twitter:url/title/description, canonical)
	are declared by each route's own <svelte:head> — declaring them here too would
	duplicate the tag on every page and search engines/social crawlers take the
	first one they find, silently overriding every page's own SEO tags with the
	homepage's. See each +page.svelte for its page-specific head.
-->
<svelte:head>
	<meta charset="utf-8" />
	...
	<!-- Open Graph / Twitter defaults — every page provides its own title/description/url -->
