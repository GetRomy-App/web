...
<svelte:head>
	...
	<!--
		Page title/description/canonical/OG-title/OG-url are intentionally NOT set here.
		SvelteKit only keeps a single <title> element when nested <svelte:head> blocks
		each declare one — the outermost (this layout) wins over every page-level
		override. Every page must declare its own complete title/description/canonical/
		OG block; the homepage's lives in src/routes/+page.svelte.
	-->
	...
	<!-- Open Graph / Twitter constants that don't vary by page -->
