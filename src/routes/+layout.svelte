...
<!--
	Only truly page-independent tags live here. Anything that must vary per route
	(title, description, keywords, canonical, og:*, twitter:title/description, and
	page-specific JSON-LD) belongs in that route's own <svelte:head> instead —
	SvelteKit renders layout head content alongside page head content rather than
	replacing it, so duplicating a tag here would shadow or conflict with every
	page's own version (e.g. every page's <title> losing out to this one).
-->
<svelte:head>
	<meta charset="utf-8" />
	...
	<!-- Open Graph / Twitter — shared across every page (no per-post images exist yet) -->
	...
	<!-- Structured Data (JSON-LD) — organization identity, present on every page -->
