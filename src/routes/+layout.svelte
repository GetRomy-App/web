<script lang="ts">
	...
	// Every route below the homepage declares its own <title>/description/canonical/OG tags in its
	// own <svelte:head>. Svelte only keeps one <title> (and search engines only trust one canonical)
	// per document, so these homepage defaults must only render on '/' — otherwise they silently win
	// over every page's own tags and every subpage ends up canonicalized to the homepage.
